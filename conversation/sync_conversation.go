...
func SyncConversation(c *gin.Context) {
	...
	respBytes, err := apiClient.GetRequest(&api_client.GetRequestOptions{
		Url:           apiClient.CoreServiceBaseURL + SyncConversationEndPoint,
	...
	if !apiCR.Success {
		//If api/sync_conversation returns success as false
		c.JSON(http.StatusInternalServerError, apiCR)
		return
	}

	//Send response with api/sync_conversation response
