...
//RemoveReaction is used remove reaction on conversation
func RemoveReaction(c *gin.Context) {
	...
	respBytes, err := apiClient.PostRequest(&api_client.PostRequestOptions{
		Url:           apiClient.CoreServiceBaseURL + RemoveReactionEndPoint,
	...
	if !apiCR.Success {
		//If api/conversation/remove_reaction returns success as false
		c.JSON(http.StatusInternalServerError, apiCR)
		return
	}

	//Send response with api/conversation/remove_reaction response
