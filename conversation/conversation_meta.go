...
//ConversationMeta is used to fetch conversation meta
func ConversationMeta(c *gin.Context) {
	...
	respBytes, err := apiClient.GetRequest(&api_client.GetRequestOptions{
		Url:           apiClient.CoreServiceBaseURL + ConversationMetaEndPoint,
	...
	if !apiCR.Success {
		//If api/conversation_meta returns success as false
		c.JSON(http.StatusInternalServerError, apiCR)
		return
	}

	//Send response with api/conversation_meta response
