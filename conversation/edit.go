...
//EditConversation is used edit a conversation
func EditConversation(c *gin.Context) {
	...
	respBytes, err := apiClient.PostRequest(&api_client.PostRequestOptions{
		Url:           apiClient.CoreServiceBaseURL + EditConversationEndPoint,
	...
	if !apiCR.Success {
		//If api/edit_conversation returns success as false
		c.JSON(http.StatusInternalServerError, apiCR)
		return
	}

	//Send response with api/edit_conversation response
