...
func SubmitPoll(c *gin.Context) {
	...
	respBytes, err := apiClient.PostRequest(&api_client.PostRequestOptions{
		Url:           apiClient.CoreServiceBaseURL + SubmitPollEndPoint,
	...
	if !apiCR.Success {
		//If api/conversation/submit_poll returns success as false
		c.JSON(http.StatusInternalServerError, apiCR)
		return
	}

	//Send response with api/conversation/submit_poll response
