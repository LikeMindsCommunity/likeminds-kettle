...
func PollUsers(c *gin.Context) {
	...
	respBytes, err := apiClient.GetRequest(&api_client.GetRequestOptions{
		Url:           apiClient.CoreServiceBaseURL + PollUsersEndPoint,
	...
	if !apiCR.Success {
		//If api/conversation/poll_users returns success as false
		c.JSON(http.StatusInternalServerError, apiCR)
		return
	}

	//Send response with api/conversation/poll_users response
