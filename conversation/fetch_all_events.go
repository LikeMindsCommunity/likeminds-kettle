...
func FetchAllEvents(c *gin.Context) {
	...
	respBytes, err := apiClient.GetRequest(&api_client.GetRequestOptions{
		Url:           apiClient.CoreServiceBaseURL + EventFetchAllEndPoint,
	...
	if !apiCR.Success {
		//If api/conversation/event/fetch_all returns success as false
		c.JSON(http.StatusInternalServerError, apiCR)
		return
	}

	//Send response with api/conversation/event/fetch_all response
