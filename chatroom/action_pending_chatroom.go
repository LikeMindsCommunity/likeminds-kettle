...
func ActionPendingChatroom(c *gin.Context) {
	...
	respBytes, err := apiClient.PostRequest(&api_client.PostRequestOptions{
		Url:           apiClient.CoreServiceBaseURL + ActionPendingChatroomEndPoint,
	...
	if !apiCR.Success {
		//If api/action_pending_chatroom returns success as false
		c.JSON(http.StatusInternalServerError, apiCR)
		return
	}

	//Send response with api/action_pending_chatroom response
