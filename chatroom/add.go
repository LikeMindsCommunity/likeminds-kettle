...
//AddChatroom is used to add members to a chatroom
func AddChatroom(c *gin.Context) {
	...
	respBytes, err := apiClient.PostRequest(&api_client.PostRequestOptions{
		Url:           apiClient.CoreServiceBaseURL + AddChatroomEndPoint,
	...
	if !apiCR.Success {
		//If api/chatroom/add returns success as false
		c.JSON(http.StatusInternalServerError, apiCR)
		return
	}

	//Send response with api/chatroom/add response
