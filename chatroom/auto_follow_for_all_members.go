...
type AutoFollowForAllMembersRequest struct {
	ChatroomID          int64 `json:"chatroom_id"`
	IncludeMembersLater bool  `json:"include_members_later"`
}

//AutoFollowForAllMembers is used to follow the chatroom for all members
func AutoFollowForAllMembers(c *gin.Context) {
	...
	//POST body params
	var affamr AutoFollowForAllMembersRequest
	if err := c.ShouldBindJSON(&affamr); err != nil {
	...
	respBytes, err := apiClient.PostRequest(&api_client.PostRequestOptions{
		Url:           apiClient.CoreServiceBaseURL + AutoFollowForAllMembersEndPoint,
